...
messagesRouter.get("/:id", async (req, res) => {
    const message = !Number.isInteger(Number(req.params.id))
        ? undefined
        : await db.getMessage(req.params.id);
    res.set({ "Content-Type": "text/html" });

    if (db.getCountMessages() < req.params.id || req.params.id <= 0 || message === undefined) {
        res.status(404);
